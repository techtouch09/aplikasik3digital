...
async function generatePDF() {
    ...
    if (tableHukum) {
        ...
        // Membersihkan teks pilihan (Contoh: "14. Jenis APAR" menjadi "jenis apar")
        ...
        rows.forEach((row, index) => {
            ...
            // --- LOGIKA KHUSUS MATERI NO 14 (APAR) ---
            let isMatch = false;

            if (selectedText.includes("14")) {
                // Jika materi no 14, cari baris yang mengandung 'kebakaran' atau 'apar'
                if (rowContent.includes("kebakaran") || rowContent.includes("apar")) {
                    isMatch = true;
                }
            } else {
                // Untuk materi selain no 14, gunakan pencocokan judul standar
                if (rowContent.includes(cleanTitle)) {
                    isMatch = true;
                }
            }
            // ------------------------------------------
            ...
            if (isHeader || isMatch) {
                ...
                cells.forEach((cell, cellIndex) => {
                    ...
                    // Penetapan Lebar Kolom
                    if (cellIndex === 0) cell.style.width = "15%"; 
                    if (cellIndex === 1) cell.style.width = "25%"; 
                    if (cellIndex === 2) cell.style.width = "25%"; 
                    if (cellIndex === 3) cell.style.width = "10%"; 
                    if (cellIndex === 4) cell.style.width = "25%"; 
...
// Fungsi untuk membuka/menutup dropdown modul
function toggleDaftarModul() {
...
// Fungsi menampilkan halaman dan reset dropdown
function showPage(event, pageId) {
    event.preventDefault();
    ...
    // Tutup dropdown secara otomatis
    const dropdown = document.getElementById("materiDropdownModul");
    const icon = document.getElementById("hamIconModul");
...
// ============================================================
// 8. FUNGSI DOWNLOAD PDF MODUL (VERSI SINGKAT)
// ============================================================
async function downloadAndNotify(sectionId, title) {
    const materiSection = document.getElementById(sectionId);
    if (!materiSection) {
        alert("Konten materi tidak ditemukan!");
        return;
    }

    // Ambil tombol yang diklik untuk animasi loading
    const btn = event.target;
    const originalText = btn.innerHTML;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mengunduh...';
    btn.disabled = true;

    // Buat wrapper sementara untuk PDF
    const pdfWrapper = document.createElement('div');
    pdfWrapper.style.padding = "30px";
    pdfWrapper.style.fontFamily = "'Arial', sans-serif";
    pdfWrapper.style.color = "#333";

    // Header Singkat
    pdfWrapper.innerHTML = `
        <div style="text-align: center; border-bottom: 2px solid #a52a2a; margin-bottom: 20px; padding-bottom: 10px;">
            <h1 style="color: #a52a2a; font-size: 20px; margin: 0;">MODUL MATERI K3</h1>
            <p style="font-size: 14px; margin: 5px 0;">Topik: ${title}</p>
        </div>
    `;

    // Ambil isi materi (Detail & Contoh) tanpa tabel landasan hukum (Versi Singkat)
    const content = materiSection.querySelector('.card').cloneNode(true);
    
    // Hapus tombol cetak agar tidak ikut ter-PDF
    const btnInClone = content.querySelector('.btn-generate');
    ...
    footer.innerHTML = `
        <div style="margin-top: 30px; border-top: 1px solid #ddd; padding-top: 10px; font-size: 10px; color: #777; text-align: center;">
            Dokumen ini dihasilkan secara otomatis oleh Sistem K3 Digital - ${new Date().toLocaleDateString('id-ID')}
        </div>
    `;
    ...
    const opt = {
        margin: [10, 10, 10, 10],
        filename: `Modul_K3_${title.replace(/\s+/g, '_')}.pdf`,
    ...
    try {
        await html2pdf().set(opt).from(pdfWrapper).save();
        // Notifikasi ke Admin (Simulasi)
        console.log(`Laporan: User telah mengunduh modul ${title}`);
    } catch (error) {
